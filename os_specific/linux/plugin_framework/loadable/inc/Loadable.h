...
public:
    ...
    public:
        ModuleSym(std::shared_ptr<ModuleHandle> owner, std::string name, T &thing)
    ...
    std::shared_ptr<ModuleSym<T>> getSym(const std::string symname) {
        ...
        if (handle) {
            ...
            if (sym != symbols.end()) {
                symbol = std::any_cast<std::weak_ptr<ModuleSym<T>>>((*sym).second).lock();
            }
            if (!symbol) {
                ...
                if (symbolp) {
                    ...
                    symbol = std::make_shared<ModuleSym<T>>(fromThis, symname, *tsymbolp);
...
protected:
    ...
    std::string defaultPath() {
        std::string ret{};
        const char *envpath = getenv("LIBS_PATH");
        std::string tmp;
        if (envpath) {
            tmp = envpath;
        }
